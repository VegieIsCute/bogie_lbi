...
{
public:
	static uint32_t unprotect(uint8_t* funcaddr)
	{
		static int total_alloc;
		static std::map<uint32_t, uint32_t> cache;
		try
		{
			uint32_t& cached_func = cache.at((uint32_t)funcaddr);
			return (uint32_t)(cached_func);
		}
		catch (std::out_of_range&)
		{
		} //cache miss, do nothing and continue

		uint32_t func_size = get_func_end(funcaddr) - funcaddr;
		if (!func_size)
		{
			return (uint32_t)(funcaddr);
		}

		if (total_alloc + func_size > max_alloc)
			return (uint32_t)(funcaddr); //failsafe, using too much memory (over 1MB)

		void* new_func = VirtualAlloc(NULL, func_size, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
		if (new_func == NULL)
			return (uint32_t)(funcaddr); //alloc failed

		total_alloc += func_size;
		memcpy(new_func, funcaddr, func_size);
		if (disable_retcheck((uint32_t)new_func, func_size))
		{
			cache.emplace((uint32_t)funcaddr, (uint32_t)new_func);
			fix_calls((uint32_t)new_func, (uint32_t)funcaddr, func_size);
			return (uint32_t)(new_func);
		}
...
private:
	...
	static bool is_prolog(uint8_t* addr)
	{
		return addr[0] == 0x55 && addr[1] == 0x8B && addr[2] == 0xEC;
	}

	static uint8_t* get_func_end(uint8_t* funcaddr) //terrible
	{
		uint8_t* addr = funcaddr + 0x10;
		while (true)
		{
	...
	static bool disable_retcheck(uint32_t new_func, uint32_t func_len)
	{
		bool has_retcheck = false;
		uint32_t disasm_ptr = new_func;
		while (disasm_ptr - new_func < func_len)
		{
			...
			{
				uint32_t ptr = disasm.disp.disp32;
	...
	static uint8_t test_function(uint32_t func)
	{
		__try
		{
			uint8_t x = *(uint8_t*)func;
			return x == 0 ? 1 : x;
		}
	...
	static void fix_calls(uint32_t new_func, uint32_t orig_func, uint32_t func_len)
	{
		uint32_t disasm_ptr = new_func;
		while (disasm_ptr - new_func < func_len)
		{
			...
			if (disasm.opcode == 0xE8) //CALL
			{
				...
				uint32_t orig_call_instr = orig_func + (disasm_ptr - new_func);
				uint32_t orig_called_func = orig_call_instr + rel_addr + 5;
				if (orig_called_func % 0x10 == 0 && test_function(orig_called_func)) //functions are aligned to 0x10 bytes
				{
					__try
					{
						uint32_t new_called_func = unprotect((uint8_t*)orig_called_func);
						uint32_t new_rel_addr = new_called_func - disasm_ptr - 5;
						*(uint32_t*)(disasm_ptr + 1) = new_rel_addr;
					}
					__except (EXCEPTION_EXECUTE_HANDLER)
					{
						//ignore it
...
{
	return (T)Retcheck::unprotect((uint8_t*)func);
}